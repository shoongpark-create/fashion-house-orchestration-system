...
from fashion_prompter import FashionPrompter
...
Examples:
  ...
  %(prog)s "portrait" --model imagen-4-ultra --aspect-ratio 3:4
    ...
    # Fashion Mode Arguments
    parser.add_argument(
        '--fashion-mode',
        choices=['lookbook', 'ghost', 'flat', 'detail'],
        help='Enable fashion prompt enhancement mode'
    )
    parser.add_argument(
        '--context',
        choices=['street', 'studio', 'nature'],
        default='studio',
        help='Background context for fashion mode'
    )
...
def main():
    ...
    try:
        ...
        # Apply Fashion Prompter if mode is selected
        if args.fashion_mode:
            print(f"👗 Fashion Mode: {args.fashion_mode} ({args.context})")
            original_prompt = args.prompt
            args.prompt = FashionPrompter.enhance(
                description=original_prompt,
                mode=args.fashion_mode,
                context=args.context
            )
            print(f"Original: {original_prompt}")
            print(f"Enhanced: {args.prompt}\n")
        else:
